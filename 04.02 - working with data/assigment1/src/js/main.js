...
const sortContactsByLastName = function (contactsArr) {
  ...
  contactsArrCopy.sort((a, b) =>
    a.name.last.localeCompare(b.name.last, { sensitivity: "base" })
  );
...
const returnTopNContactsByLastName = function (contactsArr, num) {
  return !isNaN(num) && num >= 0
    ? sortContactsByLastName(contactsArr).slice(0, num)
    : undefined;
};
...
const getContactById = function (contactsArr, { name, value }) {
  return (
    contactsArr.find(
      (item) => item.id.name === name && item.id.value === value
    ) || -1
  );
};
...
const countContactsFromCountry = function (contactsArr, countryStr) {
  return contactsArr.reduce(
    (acc, item) => acc + (item.location.country === countryStr ? 1 : 0),
    0
  );
};

// much havier than countContactsFromCountry3(), because you need to create array and iteray
const countContactsFromCountry2 = function (contactsArr) {
  return contactsArr.reduce((acc, item) => {
    let countryStr = item.location.country;
...
const countContactsFromCountry3 = function (contactsArr) {
  return contactsArr.reduce((acc, item) => {
    let countryStr = item.location.country;
    if (!acc[countryStr]) {
      acc[countryStr] = 0;
...
const contactsBetweenAges = function (contactsArr, fromAgeNum, toAgeNum) {
  return contactsArr.filter(
    (item) => item.dob.age >= fromAgeNum && item.dob.age <= toAgeNum
  );
};
